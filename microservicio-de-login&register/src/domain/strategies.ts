  ...
  public async login(email: string, password: string): Promise<{token:string,usuario:string,carrera:string} | false> {
    try {
      ...
      if (result !== undefined) {
          ...
          let compare = await brcryptjs.compareSync(password, result.contraseña);
          if (compare) {
  ...
  public async register(user:string,password:string,carrera:string): Promise<boolean> {
    ...
    try {
      ...
      if(result == undefined){
        ...
        let resultInsert = await (await mysqlGateway).insertData("INSERT INTO users (usuario,contraseña,carrera) VALUES (?,?,?)", [user,contraseñaEncriptada,carrera]);
